...
async def restock_or_not(product):
    try:
        # Use the get_db_connection function instead of global conn
        local_conn = get_db_connection()
        
        query = """SELECT * FROM "stock_table" """
        stock_table = query_to_dataframe(local_conn, query)
        local_conn.close()
...
# Add these new endpoints to your FastAPI code:

@app.get("/api/metrics/stockout-prevention")
async def calculate_stockout_prevention():
    """Calculate stockout prevention metrics"""
    try:
        conn = get_db_connection()
        query = """SELECT * FROM "stock_table" """
        ...
        for product in products:
            ...
            if stock_info["need_restock"]:
                ...
                # Assume average unit price of $50 (adjust based on your data)
                potential_lost_revenue += shortfall * 50
                
                # If we're alerting now, we're preventing the stockout
                prevented_stockouts += 1
        
        stockout_rate_without_forecast = (products_at_risk / total_products) * 100
        stockout_rate_with_forecast = max(0, (products_at_risk - prevented_stockouts) / total_products * 100)
...
async def calculate_inventory_efficiency():
    ...
    try:
        ...
        # Assume average unit cost of $30
        excess_inventory_value = total_excess_inventory * 30
        
        # Carrying cost is typically 20-25% annually
        annual_carrying_cost_saved = excess_inventory_value * 0.25
        monthly_carrying_cost_saved = annual_carrying_cost_saved / 12
        
        overstock_rate = (overstocked_products / len(products)) * 100
        understock_rate = (understocked_products / len(products)) * 100
        optimal_rate = (optimal_products / len(products)) * 100
    ...
    try:
        ...
        for product in products:
            ...
            if len(product_sales) > 0 and len(product_forecast) > 0:
                ...
                actual_last_4 = product_sales.iloc[-5:-1]["sum"].values  # -5:-1 to skip the current incomplete week
                forecast_median = product_forecast.iloc[:4]["Median Forecast"].values
                
                if len(actual_last_4) == len(forecast_median):
                    # Calculate MAPE
                    mape = np.mean(np.abs((actual_last_4 - forecast_median) / actual_last_4)) * 100
                    total_mape += mape
        ...
        avg_mape = total_mape / len(products) if len(products) > 0 else 0
        accuracy_rate = (accurate_forecasts / len(products)) * 100 if len(products) > 0 else 0
...
async def calculate_roi_summary():
    ...
    try:
        ...
        avg_emergency_order_cost = 100  # Extra cost per emergency order
